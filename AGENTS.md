...
- Pages live in `src/pages/`; current routes are `/` (`index.astro`), `/about` (`about.astro`), `/blog` (`blog.astro`), `/blog/[slug]` (`blog/[slug].astro`), and `/404` (`404.astro`).
...
- `ThemeSwitcher.astro` toggles `document.body.dataset.theme`; it is used in desktop preferences from `Header.astro` and in `MobileNav.astro`.
...
- `src/pages/404.astro`: custom not-found page with image asset and home link.
